...
const TypingCountJs = document.getElementById('typingcount');
...
const keyPress = e => {
  ...
  let counter = TypingCountJs.textContent;
  counter++;
  console.log(counter);
  TypingCountJs.textContent = counter;
...
const rankCheck = scoreJs => {
  ...
  if (scoreJs < 100) {
    text=`あなたのランクはCです。\nBランクまであと${100-scoreJs}です。`;
  }
  else if (scoreJs < 200) {
    text = `あなたのランクはBです。\nAランクまであと${200 - scoreJs}です。`;
  }
  else if (scoreJs < 300) {
    text = `あなたのランクはAです。\nSランクまであと${300 - scoreJs}です。`;
  }
  else if (scoreJs >= 300) {
    text = `あなたのランクはSです。\nおめでとうございます!`;
  }
...
startJs.addEventListener('click', () => {
  ...
  // カウント表示を表示する
  TypingCountJs.style.display = 'block';
});
